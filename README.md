# DAOOH Manuscript — Code

This repository contains the analysis code for the manuscript:

**"Statistical Modeling of Days Alive Out-of-Hospital: An Illustration Using the SSU Trial"**  

The main outcome is **Days Alive and Out-of-Hospital (DAOOH)** (a type of **Hospital-Free Days** outcome), and this repository provides scripts to reproduce the analyses, tables, and figures reported in the manuscript.


The analysis was conducted using:

- R version: 4.4.2
- Operating system: macOS Sequoia 15.7.4

### Main R packages

| Package | Version |
|---|---:|
| PResiduals | 1.0-1 |
| rms | 8.0-0 |
| Metrics | 0.1.4 |
| DHARMa | 0.4.7 |
| ggpubr | 0.6.0 |
| patchwork | 1.3.0 |
| pim | 2.0.4 |
| MASS | 7.3-61 |
| boot | 1.3-31 |
| mice | 3.17.0 |
| ggplot2 | 3.5.1 |
| Hmisc | 5.2-3 |

For the full session information, see [`sessionInfo.txt`](sessionInfo.txt).
